...
// PostgreSQLTableStatsCollector collects table statistics for PostgreSQL
type PostgreSQLTableStatsCollector struct {
	db *sql.DB
}

// NewPostgreSQLTableStatsCollector creates a new PostgreSQL table stats collector
func NewPostgreSQLTableStatsCollector(db *sql.DB) *PostgreSQLTableStatsCollector {
	return &PostgreSQLTableStatsCollector{db: db}
}

// Ensure PostgreSQLTableStatsCollector implements TableStatsCollectorInterface
var _ dbinterfaces.TableStatsCollectorInterface = (*PostgreSQLTableStatsCollector)(nil)

// GetTableStats returns detailed statistics for a specific table
func (c *PostgreSQLTableStatsCollector) GetTableStats(tableName string) (*models.TableStats, error) {
	query := `
		SELECT 
...
func (c *PostgreSQLTableStatsCollector) GetTableSizes() ([]map[string]interface{}, error) {
	query := `
		SELECT 
			schemaname,
			tablename,
...
func (c *PostgreSQLTableStatsCollector) GetSlowQueries() ([]models.SlowQuery, error) {
	query := `
		SELECT 
			query,
			calls,
